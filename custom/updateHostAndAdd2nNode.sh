...
setEnv
  ...
  until echo "blah"|nc $DMGR_HOST $DMGR_PORT ; do
    >&2 echo "Dmgr is unavailable - sleeping"
    sleep 3
  done


  until /opt/IBM/WebSphere/AppServer/bin/wsadmin.sh -lang jython -conntype SOAP -host $DMGR_HOST -port $DMGR_PORT -c "AdminTask.listNodes('[-nodeGroup DefaultNodeGroup ]')" |grep custom ; do
    >&2 echo "Dmgr is busy - sleeping"
    sleep 3
  done
